...
export class ErCaseViolationsService {
    ...
    async getViolation(erCaseViolationId: string, user: RequestUser) {
        ...
        if (!erCaseViolation || erCaseViolation.is_active === false) {
            throw new NotFoundException('Employee Relation Case Violation does not exist.');
        }
    ...
    async updateViolation(erCaseViolationId: string, dto: UpdateErCaseViolationDto, user: RequestUser) {
        // Auth check first
        const requestUser = await this.prisma.user.findUnique({
            where: { id: user.id },
            include: {
