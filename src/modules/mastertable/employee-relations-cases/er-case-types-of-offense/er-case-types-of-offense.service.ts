...
export class ErCaseTypesOfOffenseService {
    ...
    async getTypeOfOffense(erCaseTypeOfOffenseId: string, user: RequestUser) {
        ...
        const erCaseTypeOfOffense = await this.prisma.hrErCaseTypeOfOffense.findUnique({
            where: { id: erCaseTypeOfOffenseId, is_active: true, },
        })

        if (!erCaseTypeOfOffense || erCaseTypeOfOffense.is_active === false) {
            throw new BadRequestException('Employee Relation Type of Offense does not exist or is inactive');
        }
    ...
    async getTypeOfOffenses(user: RequestUser) {
        ...
        const erCaseTypeOfOffenses = await this.prisma.hrErCaseTypeOfOffense.findMany();
    ...
    async createTypeOfOffense(dto: CreateErCaseTypesOfOffenseDto, user: RequestUser) {
        // Auth check first
        const requestUser = await this.prisma.user.findUnique({
            where: { id: user.id },
            include: {
        ...
        const existingTypeOfOffense = await this.prisma.hrErCaseTypeOfOffense.findFirst({
        ...
        return {
            status: 'success',
            message: 'Employee Relations Type of Offense has been created successfully',
            typeOfOffense,
        };
    }

    async updateTypeOfOffense(erCaseTypeOfOffenseId: string, dto: UpdateErCaseTypesOfOffenseDto, user: RequestUser) {
        // Auth check first
        const requestUser = await this.prisma.user.findUnique({
            where: { id: user.id },
            include: {
        ...
        const updateErCaseTypeOffense = await this.prisma.hrErCaseTypeOfOffense.update({
            where: { id: erCaseTypeOfOffenseId, },
            data: {
                type_of_offense: dto.type_of_offense ?? erCaseTypeOfOffense.type_of_offense,
        ...
        return {
            status: 'success',
            message: 'Employee Relation Case Type of Offense has been updated successfully',
