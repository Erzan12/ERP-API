...
import {
  CreatePerformanceCompetencyDto,
  UpdatePerformanceCompetencyDto,
} from './dto/performance-competency.dto';
...
export class PerformanceCompetencyService {
  ...
  async getCompetencies(user: RequestUser) {
    ...
    if (competencies.length === 0) {
      throw new NotFoundException(
        'No Performance competencies currently available or added',
      );
    }
  ...
  async createCompetencies(
    user: RequestUser,
    dto: CreatePerformanceCompetencyDto,
  ) {
    const {
      department_group,
      sea_category,
  ...
  async updateCompetency(
    competencyId: string,
    user: RequestUser,
    dto: UpdatePerformanceCompetencyDto,
  ) {
    // Authorization Check
    const requestUser = await this.prisma.user.findUnique({
      where: { id: user.id },
    ...
    const existingCompetency =
      await this.prisma.hrPerformanceCompetency.findUnique({
        where: { id: competencyId },
      });
