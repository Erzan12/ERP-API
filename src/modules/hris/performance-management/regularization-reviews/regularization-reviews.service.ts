...
export class RegularizationReviewsService {
  ...
  async getForRegularization(user: RequestUser, dto: RegularizationReviewDto) {
    ...
    const whereCondition: Prisma.RegularizationEligibilityWhereInput = {};
    if (search) {
      whereCondition.OR = [
        {
          employee: {
            person: { first_name: { contains: search, mode: 'insensitive' } },
          },
        },
        {
          employee: {
            person: { last_name: { contains: search, mode: 'insensitive' } },
          },
        },
        {
          employee: {
    ...
    const sortOrder =
      sortMap[sortBy as keyof typeof sortMap] || sortMap.employee_created_at;
  ...
  async createEvaluation(dto: CreateEvaluationDto, user: RequestUser) {
    ...
    const fullEvaluation = await this.prisma.hrEmployeeEvaluation.findUnique({
    ...
    if (!fullEvaluation) {
      throw new BadRequestException('Evaluation not found after creation');
    }

    // Return with computed status
    return {
      ...evaluation,
      status: computeEvaluationStatus(fullEvaluation),
    };
  }
  ...
  async getEmployeeEvaluations(employeeId: string, user: RequestUser) {
    ...
    if (evaluations.length === 0) {
      throw new NotFoundException('No employee evaluation yet.');
    }
  ...
  async getEvaluations(user: RequestUser, dto: RegularizationReviewDto) {
    ...
    for (const evalItem of evaluations) {
      if (!grouped.has(evalItem.employee_id))
        grouped.set(evalItem.employee_id, []);
      grouped.get(evalItem.employee_id)!.push(evalItem);
    }

    // let processedResults = [];
    let processedResults: ProcessedEvaluation[] = [];
    ...
    return this.prisma.$transaction(async (tx) => {
      try {
        const rejectEvaluation = await tx.hrEmployeeEvaluation.update({
          where: {
            id: employeeEvaluationId,
            OR: [{ status: 'for_verification' }, { status: 'for_approval' }],
          },
          data: {
            status: 'rejected',
            updated_by: requestUser.id,
