...
export class LeaveCasesService {
  ...
  async createLeaveCase(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const inputDates = leave_dates.map(
      ...
      const uniqueDates = new Set(leave_dates.map((d) => d.leave_date));
  ...
  async updateLeaveCase(
    user: RequestUser,
    leaveCaseId: string,
    dto: UpdateLeaveRequestWithDetailsDto,
  ) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (update_leave_dates && update_leave_dates.length > 0) {
        for (const d of update_leave_dates) {
          if (d.id) {
            ...
            if (!d.leave_date) {
              throw new BadRequestException(
                'leave_date is required for new date entries',
              );
            }
            if (!d.leave_compensation) {
              throw new BadRequestException(
                'leave_compensation is required for new date entries',
              );
            }
      ...
      if (update_leave_request.verifier_id) {
        ...
        if (currentVerificationStep) {
          ...
          if (
            ...
            update_leave_request.verifier_id
          ) {
      ...
      if (update_leave_request.approver_id) {
        ...
        if (currentApprovalStep) {
          ...
          if (
            currentApprovalStep.acted_by !== update_leave_request.approver_id
          ) {
            await tx.workflowAction.update({
      ...
      const updatedLeaveCase = await tx.hrLeaveRequest.update({
        ...
        data: {
          leave_category_id:
            update_leave_request.leave_category_id ?? undefined,
          date_from: update_leave_request.date_from
            ? new Date(update_leave_request.date_from)
            : undefined,
