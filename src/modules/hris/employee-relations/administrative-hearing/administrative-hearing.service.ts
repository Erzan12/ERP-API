...
import { HrErCasePartyRole, HrErCaseStage, HrErHearingStatus, Prisma } from '@prisma/client';
import { PrismaService } from 'src/config/prisma/prisma.service';
import { RescheduleHearingDto, ScheduleHearingDto } from './dto/schedule-hearing.dto';
...
export class AdministrativeHearingService {
    ...
    private async assertRespondentAtStage(
        ...
        if (party.stage !== expectedStage) {
            throw new BadRequestException(`Party is at stage "${party.stage}", not ${expectedStage}.`);
        }
    ...
    async scheduleHearing(dto: ScheduleHearingDto, user: RequestUser) {
        ...
        return this.prisma.$transaction(async (tx) => {
            await this.assertRespondentAtStage(tx, dto.disciplinary_case_id, dto.party_id, HrErCaseStage.administrative_hearing)
            ...
            if (activeScheduled) {
                throw new ConflictException('A hearing is already scheduled - use reschedule instead.');
            }
    ...
    async rescheduleHearing(disciplinaryCaseId: string, partyId: string, dto: RescheduleHearingDto, user: RequestUser) {
        await this.assertHrAccess(user.id);

        return this.prisma.$transaction(async (tx) => {
            await this.assertRespondentAtStage(tx, disciplinaryCaseId, partyId, HrErCaseStage.administrative_hearing);
            ...
            if (!activeScheduled) {
                throw new BadRequestException('No active scheduled hearing to reschedule - use schedule instead.');
            }
    ...
    async conductHearing(disciplinaryCaseId: string, partyId: string, dto: ConductHearingDto, user: RequestUser) {
        await this.assertHrAccess(user.id);
