...
export class SubmitDecisionDto {
    ...
    @ValidateIf((o) => DATED_DECISION_TYPES.includes(o.decision_type))
    @IsDateString()
    @ApiProperty({ required: false, example: '2026-08-01', description: 'Required for suspension/rehabilitation decisions' })
    effectivity_start?: string;

    @ValidateIf((o) => DATED_DECISION_TYPES.includes(o.decision_type))
    @IsDateString()
    @ApiProperty({ required: false, example: '2026-08-03', description: 'Required for suspension/rehabilitation decisions' })
    effectivity_end?: string;

    @IsOptional()
    @IsString()
    @ApiProperty({ required: false, description: 'URL of the signed Notice of Decision' })
