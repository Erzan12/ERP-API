...
import { IssueNteDto, ReviewNteApprovalDto } from './dto/notice-of-explaination.dto';
import { RequestUser } from 'src/utils/types/request-user.interface';
import { HrErApprovalStatus, HrErApprovalStepType, HrErCasePartyRole, HrErCaseStage } from '@prisma/client';
...
export class NoticeOfExplainationService {
    ...
    async issueNte(disciplinaryCaseId: string, partyId: string, dto: IssueNteDto, user: RequestUser) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (party.stage !== HrErCaseStage.notice_to_explain) {
                throw new BadRequestException(`Party is at stage "${party.stage}", not notice_to explain`);
            }
            ...
            if (existing?.issued_at) {
                throw new ConflictException('An NTE has already been issued for this respondent.');
            }
            ...
            if (reviewers.length !== dto.reviewer_ids.length) {
                throw new BadRequestException('One or more reviewer_ids do not match an existing user.');
            }
    ...
    async reviewNteApproval(
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (approval.step_type !== HrErApprovalStepType.nte_review || !approval.nte) {
                throw new BadRequestException('This approval is not an NTE review step.');
            }
            ...
            if (approval.status !== HrErApprovalStatus.pending) {
                throw new ConflictException(`This approval was already marked "${approval.status}.`);
            }

            await tx.hrErCaseApproval.update({
                where: { id: approvalId },
                data: { status: dto.status, remarks: dto.remarks, reviewed_at: new Date() },
            });

            // Role update the overall NTE status from all reviewers on this NTE
            const siblingApprovals = await tx.hrErCaseApproval.findMany({ where: { nte_id: approval.nte.id } });
            const updated = siblingApprovals.map((a) => (a.id === approvalId ? { ...a, status: dto.status } : a));
