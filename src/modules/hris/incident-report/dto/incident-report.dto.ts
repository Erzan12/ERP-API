...
import { ArrayMinSize, ArrayNotEmpty, IsDateString, IsNotEmpty, IsOptional, IsString, IsUUID, ValidateNested } from "class-validator";
...
export class CreateIncidentReportDto {
    ...
    @IsString()
    @IsOptional()
    @ApiProperty({ required: false, example: 'The personnel jump off the vessel' })
...
export class UpdateIncidentReportDto {
    ...
    @IsString()
    @IsOptional()
    @ApiPropertyOptional({ required: false, example: 'The personnel jump off the vessel' })
    incident_narrative?: string;
