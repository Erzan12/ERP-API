...
import { IsEnum, IsNotEmpty, IsString, IsUUID, IsDateString, IsInt } from "class-validator";
...
export class AssignInterviewerDto {
    ...
    @IsString()
    @IsNotEmpty()
    @IsEnum(InterviewStage, { message: 'Interview stage must be Initial, Second, and Final' })
    @Type(() => String)
    @ApiProperty({
        enum: InterviewStage,
