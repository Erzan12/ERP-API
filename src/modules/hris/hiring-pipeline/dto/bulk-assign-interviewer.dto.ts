...
import { ArrayMaxSize, ArrayMinSize, IsArray, IsNotEmpty, IsUUID, IsDateString, IsString, IsOptional } from "class-validator";

export class BulkAssignInterviewDto {
    @IsUUID()
    @IsNotEmpty()
    @ApiProperty({ example: 'Applicant uuid', description: 'The uuid of the applicant' })
    applicant_id: string;
    ...
    @IsDateString()
    @IsNotEmpty()
    @ApiProperty({ example: '', description: 'The date of interview of the applicant' })
