...
import { ArrayNotEmpty, IsArray, IsInt, IsNotEmpty, IsString, IsOptional } from "class-validator";
...
export class UpdatePermissionTemplateDto {
    ...
    @IsInt({ each: true })
    @IsOptional()
    @ApiProperty({ example: 1, description: 'ID of the position for this permission template' })
    position_id?: number;
